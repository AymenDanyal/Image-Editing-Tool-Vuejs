    ...
    <div class="setting-item" v-show="waterMarkState.position === 'full'">
      ...
      <div>
        <RadioGroup v-model="waterMarkState.isRotate">
          <Radio :label="0">Horizontal</Radio>
          <Radio :label="1">Tilt</Radio>
        </RadioGroup>
      </div>
    </div>
  </Modal>
...
const waterMarkState = reactive({
  ...
  font: 'serif', // Consider custom fonts
  color: '#ccc', // Consider custom colors
  position: 'lt', // lt upper left lr upper right lb lower left rb lower right full tile
});
...
const drawWaterMark: Record<string, any> = {
  ...
  full: (width: number, height: number, cb: (imgString: string) => void) => {
    ...
    let patternCanvas: HTMLCanvasElement | null = createCanvas(
      waterMarkState.isRotate === 0 ? textW : textW * 2, // If there is an inclination, then the hypotenuse will be larger than the right-angled side. Calculate 2 times at 30 degrees (simple)
    ...
    if (waterMarkState.isRotate === 0) {
      ...
      ctxWater.rotate((-30 * Math.PI) / 180); // Simple example: Calculate according to 30 degrees
...
const onModalOk = () => {
  ...
  drawWaterMark[waterMarkState.position](width, height, (imgString: string) => {
    canvasEditor.canvas.overlayImage = null; // clear overlay
