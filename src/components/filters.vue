...
// No parameter filter modification status
...
// There are parameter and combination filter modifications
const changeFiltersByParams = (type) => {
  ...
  if (moduleInfo.status) {
    // Combined parameter filter modification
    if (moduleInfo.handler) {
      _changeAttrByHandler(moduleInfo);
    } else {
      // There are parameter filter modifications
